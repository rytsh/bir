<script lang="ts">
  ...
  // Determine IP type
  const getIpType = (ipAddress: string): { type: string; description: string } | null => {
    ...
    if (ipAddress.includes(":")) {
      ...
      if (ipAddress.startsWith("fc") || ipAddress.startsWith("fd")) {
        return { type: "Private", description: "IPv6 unique local address (ULA)" };
      }
    ...
    if (a === 127) {
      return { type: "Loopback", description: "Local loopback address (127.0.0.0/8)" };
    }
    if (a === 10) {
      return { type: "Private", description: "Private Class A (10.0.0.0/8)" };
    }
    if (a === 172 && b >= 16 && b <= 31) {
      return { type: "Private", description: "Private Class B (172.16.0.0/12)" };
    }
    if (a === 192 && b === 168) {
      return { type: "Private", description: "Private Class C (192.168.0.0/16)" };
    }
    if (a === 169 && b === 254) {
      return { type: "Link-Local", description: "APIPA address (169.254.0.0/16)" };
    }
...
<div class="h-full flex flex-col">
  ...
  <div class="flex-1 flex flex-col gap-4">
    <!-- IP Display Card -->
    <div class="p-6 bg-(--color-bg-alt) border border-(--color-border)">
      ...
      {#if loading}
        <div class="flex items-center justify-center py-8">
          <div class="flex items-center gap-3">
            <div class="w-5 h-5 border-2 border-(--color-primary) border-t-transparent rounded-full animate-spin"></div>
            <span class="text-(--color-text-muted)">Fetching IP address...</span>
          </div>
        </div>
      {:else if error}
        <div class="p-4 bg-(--color-error-bg) border border-(--color-error-border) text-(--color-error-text) text-sm">
      ...
      {:else if ip}
        <div class="space-y-4">
          <!-- IP Address Display -->
          <div class="flex items-center justify-between gap-4 p-4 bg-(--color-bg) border border-(--color-border)">
            <div class="flex-1 min-w-0">
              <span class="text-2xl sm:text-3xl font-mono text-(--color-text) break-all">{ip}</span>
            </div>
            <button
              onclick={() => handleCopy("ip", ip || "")}
              class="px-3 py-2 text-sm bg-(--color-primary) hover:bg-(--color-primary-hover) text-white transition-colors shrink-0"
            >
          ...
          {#if ipInfo}
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div class="p-3 bg-(--color-bg) border border-(--color-border)">
                <span class="text-xs text-(--color-text-muted) block mb-1">Type</span>
                <span class="text-sm font-medium text-(--color-text)">{ipInfo.type}</span>
              </div>
              <div class="p-3 bg-(--color-bg) border border-(--color-border)">
                <span class="text-xs text-(--color-text-muted) block mb-1">Classification</span>
                <span class="text-sm text-(--color-text)">{ipInfo.description}</span>
              </div>
            </div>
          {/if}
          ...
          <!-- IP Version -->
          <div class="p-3 bg-(--color-bg) border border-(--color-border)">
            <span class="text-xs text-(--color-text-muted) block mb-1">Version</span>
  ...
  <div class="mt-4 p-3 bg-(--color-bg-alt) border border-(--color-border) text-sm text-(--color-text-muted)">
    ...
    Your public IP address is how external servers identify your connection on the internet.
    This is the address visible to websites and services you connect to. If you're behind a
    NAT router or VPN, this shows the external-facing address, not your local network IP.
  </div>
</div>
